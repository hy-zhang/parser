...
object Typed {
  ...
  trait Parser[E, T, F <: {val pE : PackratParser[E]; val pT : PackratParser[T]}] {
    lazy val pAbsArr: AbsArrAlg[E, T] => (=> F) => PackratParser[E] = alg => l => {
      ...
      ("\\" ~> lcid) ~ (":" ~> t) ~ ("." ~> e) ^^ { case x ~ t0 ~ e0 => alg.lam(x, t0, e0) } ||| "(" ~> e <~ ")"
    }
...
trait SimpleBoolParser[E, T, L <: {val pE : Util.PackratParser[E]; val pT : Util.PackratParser[T]}]
  extends Typed.Lexer with Bool.Lexer with BoolType.Lexer {
  val pTypedET = new Typed.Parser[E, T, L]() {}
  val pSimpleBoolLNGE = pTypedET.pE | new Bool.Parser[E, L]() {}.pE
  val pSimpleBoolLNGT = pTypedET.pT | new BoolType.Parser[T, L]() {}.pT
}

trait SimpleBoolAlg[E, T] extends Typed.Alg[E, T] with Bool.Alg[E] with BoolType.Alg[T]

trait SimpleBoolPrint extends SimpleBoolAlg[String, String]
  with Typed.Print with Bool.Print with BoolType.Print
