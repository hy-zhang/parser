...
\begin{itemize}
\item \textbf{Extension 1}: adding a new case (or rather, a new construct) to \inlinecode{Expr};
\item \textbf{Extension 2}: adding a new operation to \inlinecode{Expr}.
\end{itemize}
...
\lstinputlisting[linerange=30-32]{../Scala/Parser/src/PaperCode/Sec3Inheritance/Code1.scala}% APPLY:linerange=INHERITANCE_SIMPLE_LAM
...
\huang{Extending from old parser is necessory to get the `pExpr', added a surrounding trait.}
\lstinputlisting[linerange=43-46]{../Scala/Parser/src/PaperCode/Sec3Inheritance/Code1.scala}% APPLY:linerange=INHERITANCE_BAD_ATTEMPT
...
hence the parser does not work as expected. Still, one may continue to change the recursive call in \inlinecode{pAdd} with \inlinecode{pVarExpr},
...
\huang{It seems a little bit weird since we were already using inheritance in the `bad attempt'. I think we could focus on overriding here}
...
\lstinputlisting[linerange=50-55]{../Scala/Parser/src/PaperCode/Sec3Inheritance/Code1.scala}% APPLY:linerange=INHERITANCE_APPROACH
...
actually refers to \inlinecode{this.pExpr} again, due to dynamic dispatch. It implies that the recursive call has included the extension. This
...
\lstinputlisting[linerange=60-66]{../Scala/Parser/src/PaperCode/Sec3Inheritance/Code1.scala}% APPLY:linerange=MULTIPLE_INHERITANCE
