...
syntax. We compared our implementation with a non-modular implementation we found online [], which is also written in Scala and uses the same Packrat parsing library. The comparison is made from two aspects, source lines of code (SLOC) and execution time. \huang{done}\bruno{mention that you compare against an existing
