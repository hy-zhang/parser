...
\end{lstlisting}
In Scala, however, there is no library \lstinline{fix} function, but
it can be easily implemented since Scala also supports lazy
...
\end{lstlisting}
where function \lstinline{f} has a by-name parameter of type
\lstinline{"=>} \lstinline{A"}.  Here we illustrate by the famous
Fibonacci example. Traditionally it can be implemented in Scala using
pattern-matching:
