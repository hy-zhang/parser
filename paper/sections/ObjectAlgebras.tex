...
be reusable. \lstinline{VarExprParser} is no longer suitable for parsing the new ASTs! All the methods in the previous parser has type \inlinecode{Parser[Expr]}, which indicates the result has type \inlinecode{Expr}, whereas the type should be \inlinecode{FVarsExpr} now.
