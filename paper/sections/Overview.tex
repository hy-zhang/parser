...
\bruno{Write a summary for this section}
\bruno{Fill in all the missing references}
...
\huang{done, the example may be not very good, but i haven't found a
  better one}
...
\bruno{The example is good, I think but the explanation is not. 
...
they support left-recursive grammars directly. All of these properties are very
...
changed, we can not apply them directly. The client code below shows
how to use the new parser. The \inlinecode{fix} function, which is the
the standard \textit{fixpoint combinator}, is used to `close' the open
recursion. \bruno{Too much detail is being given here about open
