...
\huang{a paragraph here, refer to related work section, discuss
  ...
  drawbacks}\bruno{Please finish this.}
...
\bruno{You are not motivating the problem! You are going straight to
...
\bruno{I think we need to set up a challenge here, similar to the challange of
...
\end{lstlisting}
\bruno{if you intend to write a grammar here, then please use grammar
  (BNF) notation, and not some notation invented by you!}
...
knows how to use it!}
\bruno{Please polish up code. You probably want to consistently use 
...
\bruno{Also, for consistency, why isn't ``pVar'' open? I.e. using a
...
\bruno{You should use a more OO approach in the example. 
...
\paragraph{Parsing Components}\bruno{consider removing and/or moving
...
\end{lstlisting}
\bruno{Fix is not the right name. A better name is Open (for open
  recursion).}
\bruno{I'm not sure that showing the code for language components is
  essential at this point. This can be shown latter in the paper.}
...
\end{lstlisting}
\bruno{same comment as before: if this is supposed to be a grammar,
  then use a grammar notation}
