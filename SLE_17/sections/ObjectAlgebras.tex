...
be reusable. \lstinline{VarParser} is no longer suitable for parsing the new ASTs! All the methods in the previous parser have type \inlinecode{Parser[Expr]},
which generates ASTs that only support pretty-printing. Now we have to substitute \inlinecode{NewExpr} for \inlinecode{Expr} (the
same for \inlinecode{Lit}, \inlinecode{Add}, and so on). It requires either modification on existing code or casts.
