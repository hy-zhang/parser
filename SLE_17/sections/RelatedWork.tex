...
Besides, type-specific languages~\cite{omar14} do support
separate compilation and type safety. The modularity of parsing is incorporated in its type
system, which allows users to introduce new syntax for parsing. Whereas in our approach, parsing 
strategy is an orthogonal issue since we rely on an external parsing library.
Our approach does not rely on an elaborated semantics extension, but simply uses a pattern
that is applicable in mainstream OO languages, where the type system helps ensure
type safety.
...
\emph{separate compilation} as well as \emph{modular
  type-checking}, and is applied in common OO languages. Moreover, the extensibility of parsing is further
